...
# * 164 3.1.3 /Fixed/ลืมและแก้ยังไงไป  /BUG/ ให้ customer class มันข้ามมารอโดยที่ drop down ไม่ออกด้วยซ้ำ มันขาด loop เรอะ
# ! 165 /Adjust/ มันมีเครื่องหมาย ? แฝงมา มันเกิดขึ้นแม้แต่กับตัวที่เพิ่ง add เข้ามา ตัวอย่างจาก order นี้ 25090926K9A7CB ทำให้หลัง add แทนที่จะไปได้เลย กลับกลายเป็ฯต้องแก้ชื่อ เพราะตอนเช็คมันเจอ ?  compare self.current_address & self.desired_full_address ตัวอย่าง 222/784หมู่?ที่?9บางบ่อบางบ่อสมุทรปราการ10560 & 222/784หมู่ที่9บางบ่อบางบ่อสมุทรปราการ10560
# ? 166 /fixed/ ปรับ while ช่วง add จากเดิมใช้ เช็คจาก threading เป็น whle true แทน และ ก่อนเริ่ม threading ใหม่ ให้ set() thread ก่อนเริ่มใหม่เพื่อให้หยุดการทำงาน /Bug Hint/ การใช้ while not self.operation_thread.is_set() เพื่อดักการทำงานของ thread มันอาจจะทำให้ threadใหม่ ดับ ซึ่งเกิดมาจาก thread ใหมที่ที่กำลังเริ่ม พร้อมกับ thread เก่าที่กำลังถูกตัด
# ! 167 3.1.4 /Bug/ หน้า add ลูกค้ามันกรอกค่าใหม่ทุกๆ 50 วิ อันนี้ผมอยากลองของเองมันเลยพัง
# ? 168 3.1.4 /Refactor/ ปรับรวม function Add ลูกค้าของ ใบกำกับ ธรรมดา และ ใบกำกับLaz เข้าเป็น 1เดียว
# ! 169 3.1.4 /Bug/ มันมีการวนมา Add ชื่อซ้ำหลังจาก Add ไปแล้ว
