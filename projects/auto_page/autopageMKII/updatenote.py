...
# * 225 /Fixed/ 4.2.2(LITEonly) ลองเอา CB ออกละ เพราะ cb มันจะเรียก get_cus_name_ready() ที่ซึ่งเปน process หลักของการหาชื่อลูกค้า ทำให้มี process หลักซ้อนกันสองอัน /Issue/ select_cus_name_from_lis() มีปัญหามันจะใช้ cb ซ้ำ เพราะมันเหมือนจะไม่ได้ตรวจว่าชื่อมันถูกแล้ว งง ทำไมก่อนหน้านี้มันไม่บัค
# * 226 /Fixed/ 4.2.1 function ซิ่ง มีโอกาสที่จะทำให้ ตัว form submit bill มันบันทึกยอดออกบิลก่อน validate ได้หากใส่ยอดเกิน
# * 227 /Fixed/ 4.2.2(LITEonly) ปรับ interval ของ ตัวรอ response ใหม่จาก 0.1 เป็น 1.0 ซึ่งหน่วยน่าจะ sec มั้ง ทำให้มันกรอกทันตลอดแล้วจากที่เทสมา 35orders /ISSUE/ ค่าขนส่งบางครั้งมันไม่แอดมันข้ามมารอขั้นตอนท้ายเลย
# * 228 /Fixed/ 4.2.2(LITEonly) ค่าขนส่งจะถูกตรวจสอบจาก response ว่า มีมี reponse ยังถ้ามีค่อยปรับราคา/ISSUE/ ค่าขนส่งไม่ถูกปรับหลังจาก input ลงไปที่ smco
# * 229 /Fixed/ 4.2.2(LITEonly) ใช้ selenium กด เพราะมีพฤติกรรมคล้ายมนุษย์ทำให้ไม่สามารถกดปุ่มที่ไม่ควรกดได้เร็วเกินไป /ISSUE/ การใช้ js กด submit มันอันตรายเกินไป
...
# * 237 /Fixed/ 4.2.2(LITEonly) tax_name_standardizer() เปลียนชื่อเปน tax_name_formatter() และเพิ่มความสามารถในการจัดการลกค้าที่ขอมาเปน บมจ. ด้วย เช่น "บมจ. ความอดทนมี" เป็น "บริษัท ความอดทนมี จำกัด (มหาชน)"
