...
def categorize_reason(reason):
    r = str(reason)
    if 'Postal code' in r and 'cannot be found in dropdown' in r:
        return '1. Postal Code Not Found in Dropdown'
    elif 'ราคา/จำนวนไม่ตรงหลังปรับราคา' in r or 'ขอวิธีปรับราคาครับ' in r or 'Price mismatch' in r:
        return '2. Price / SKU Amount Mismatch (ขอวิธีปรับราคา)'
    elif 'ไม่มีสินค้าให้ตรวจสอบและปรับราคา' in r:
        return '3. No Product to Check/Adjust Price (ไม่มีสินค้าใน Accel/บิล)'
    elif 'ไม่พบออเดอร์' in r and 'ในระบบ Shopee' in r:
        return '4. Order Not Found in Shopee'
    else:
        return '5. Other Errors / System Exceptions'
...
def load_data():
    ...
    combined['category'] = combined['failed_reason'].apply(lambda x: categorize_reason(x) if x else '')
    return combined
...
print("=== OVERALL PARETO CATEGORIZATION ===")
...
print(f"Total Failures across both files: {total_failures}")
...
print("\n=== CATEGORY BREAKDOWN BY DATE ===")
df_failed['date'] = df_failed['timestamp'].dt.date
ct_date = pd.crosstab(df_failed['category'], df_failed['date'], margins=True)
print(ct_date.to_string())

print("\n=== CATEGORY BREAKDOWN BY SESSION ===")
ct_sess = pd.crosstab(df_failed['session_id'], df_failed['category'])
print(ct_sess.to_string())

print("\n=== SESSION SUCCESS RATES & DOMINANT FAILURE CATEGORY ===")
for sess_id, group in df.groupby('session_id'):
    ...
    sess_failed_df = group[group['status'] == 'Failed']
    dom_cat = sess_failed_df['category'].value_counts().to_dict() if fail > 0 else "None"
    
    print(f"Session {sess_id:02d} | Date: {start_t.strftime('%Y-%m-%d')} | Time: {start_t.strftime('%H:%M')} - {end_t.strftime('%H:%M')} | Total: {tot:3d} | Comp: {comp:3d} | Fail: {fail:2d} | Succ: {succ:6.2f}% | Fail breakdown: {dom_cat}")
