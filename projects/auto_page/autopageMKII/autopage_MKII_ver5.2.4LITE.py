...
class MyApp:
    ...
    def create_main_window(self):
        ...
        self.root.title("Autosamatic ver5.2.4LITE")
    ...
    _MODE_COLORS = {
        "test": {
            "bg":    "#1a3a5c",   # navy blue
            "title": "Autosamatic ver5.2.4LITE  ⚠️ TEST MODE",
        },
        "real": {
            "bg":    "#444444",   # charcoal
            "title": "Autosamatic ver5.2.4LITE",
        },
    }
    ...
    def _order_search_internal(self, order,  on_complete, my_gen=None):
        ...
        if self.order != "":
            ...
            if not self.data_frame[(self.data_frame["หมายเลขคำสั่งซื้อ"] == self.order)].empty:
                ...
                # * Single Source of Truth (SSOT) สำหรับการคำนวณราคาและส่วนลด
                self.financials = OrderFinancials(
                    items=self.items,
                    shipping_cost=float(self.cus_ship_cost.get() or 0),
                    seller_voucher=float(self.cus_seller_voucher.get() or 0),
                    marketplace=self.marketplace_target.get()
                )
                self.net_prices_list = [
                    self.financials.item_expected_prices.get(
                        str(it.get('เลขอ้างอิง SKU (SKU Reference No.)', '')).strip(), 0
                    ) for it in self.items
                ]
                self.sum_price = self.financials.sum_price
                ...
                # * จากปัญหาข้อที่ 37 // การอัพเดท LOG เนื่องจาก LAZ กับ Shopee มีเงื่อนไข การใส่ค่าขนส่งในการออกบิลไม่เหมือนกัน
                if self.marketplace_target.get() == "SHOPEE":
                    self.update_log(f"สินค้ารวมค่าส่ง หักseller: {self.f(self.financials.final_billing_price)}")
                elif self.marketplace_target.get() == "LAZADA":
                    self.update_log(f"สินค้าเฉยๆ หักseller: {self.f(self.financials.final_billing_price)}")
                    self.update_log(f"---------------------------------")
...
class Bot_POS:
    ...
    def sku_formater(self, sku_input: str):
        return self.pricing_reconciler.sku_formater(sku_input)

    def oc_amounts_calculator(self, entered_data):
        return self.pricing_reconciler.oc_amounts_calculator(entered_data)

    def smco_set_overcharge_product(self, items_user_input: str = None, oc_amounts_input: str = None):
        return self.pricing_reconciler.smco_set_overcharge_product(items_user_input, oc_amounts_input)

    def smco_set_discount_product(self, items_user_input: str = None, dc_amounts_input: str = None, qty=None):
        return self.pricing_reconciler.smco_set_discount_product(items_user_input, dc_amounts_input, qty)
