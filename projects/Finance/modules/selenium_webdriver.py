...
class ChromeDriver:
    ...
    def get_pdf_src_and_print(self):
        ...
        utz_time = datetime.datetime.now()
        tz = pytz.timezone('Asia/Bangkok')
        th_time = utz_time.astimezone(tz).strftime("%d_%m_%Y-%I_%M_%S_%p")

        try:
            # file_name = f"online_inv_output_{th_time}.pdf"
            # todo printing command is the code below
            print("printing the selected inv")
            with open(f"online_inv_output_{th_time}.pdf", "wb") as pdf_file:
                pdf_file.write(bin_pdf_data)
                os.startfile(f"online_inv_output_{th_time}.pdf", "print")
